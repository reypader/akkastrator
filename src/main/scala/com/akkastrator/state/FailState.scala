...
import com.akkastrator.state.common.States.{Action, Decision, State, TransactionContext}
...
case class FailState(error: String, cause: String, comment: Option[String] = None)
  ...
  override def decide(context: TransactionContext, data: JsonNode): Try[Decision] = Try {
    Decision(context.copy(data = States.PARSER.parse(data), currentState = States.END))
  }
}